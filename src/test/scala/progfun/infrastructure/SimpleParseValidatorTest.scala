...
class SimpleParseValidatorTest extends AnyFunSuite {
  val parseValidator = new SimpleParseValidator(
    "./src/test/resources/testLawn.txt"
  )
  ...
  test("testValidateLawnDefinition") {
    val method = parseValidator.getClass
      .getDeclaredMethod("validateLawnDefinition", classOf[String])
  ...
  test("testValidateMowersData") {
    val method = parseValidator.getClass
      .getDeclaredMethod("validateMowersData", classOf[List[String]])
    method.setAccessible(true)
    val result = method.invoke(
      parseValidator,
      List("1 2 N", "GAGAGAGAA", "3 3 E", "AADAADADDA")
    )
    assert(result == Success(()))
  }
